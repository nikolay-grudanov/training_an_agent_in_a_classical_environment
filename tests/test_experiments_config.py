...
class TestConfiguration:
    ...
    def test_init_full(self) -> None:
        ...
        assert config.hyperparameters["learning_rate"] == 1e-3  # Пользовательское значение
    ...
    def test_invalid_training_steps(self) -> None:
        ...
        with pytest.raises(ValidationError, match="training_steps должен быть положительным"):
            Configuration(
                algorithm="PPO", 
                environment="LunarLander-v2", 
                training_steps=0
    ...
    def test_invalid_evaluation_frequency(self) -> None:
        ...
        with pytest.raises(ValidationError, match="evaluation_frequency должен быть положительным"):
            Configuration(
                algorithm="PPO",
                environment="LunarLander-v2",
        ...
        with pytest.raises(ValidationError, match="evaluation_frequency не может быть больше"):
            Configuration(
                algorithm="PPO",
                environment="LunarLander-v2",
                training_steps=1000,
    ...
    def test_empty_experiment_name(self) -> None:
        ...
        with pytest.raises(ValidationError, match="experiment_name не может быть пустым"):
            Configuration(
                algorithm="PPO",
                environment="LunarLander-v2",
                experiment_name=""
    ...
    def test_hyperparameter_validation(self) -> None:
        ...
        with pytest.raises(ValidationError, match="learning_rate должен быть положительным"):
            Configuration(
                algorithm="PPO",
                environment="LunarLander-v2",
