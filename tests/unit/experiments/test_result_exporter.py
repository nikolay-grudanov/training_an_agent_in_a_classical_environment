...
class TestResultExporter:
    ...
    def test_export_experiment_json(
        self, mock_tracker, temp_output_dir, mock_experiment
    ):
        ...
        result = exporter.export_experiment(
            mock_experiment, formats=[ExportFormat.JSON]
        )
    ...
    @patch("src.experiments.result_exporter.DependencyTracker")
    def test_export_experiment_csv(
        self, mock_tracker, temp_output_dir, mock_experiment
    ):
        ...
        result = exporter.export_experiment(mock_experiment, formats=[ExportFormat.CSV])
    ...
    @patch("src.experiments.result_exporter.DependencyTracker")
    def test_export_experiment_multiple_formats(
        self, mock_tracker, temp_output_dir, mock_experiment
    ):
    ...
    def test_export_with_dependencies(
        self, mock_tracker, mock_snapshot, temp_output_dir, mock_experiment
    ):
    ...
    def test_incremental_export(self, mock_tracker, temp_output_dir, mock_experiment):
        ...
        incremental_result = exporter.incremental_export(
            mock_experiment, export_dir, update_existing=True
        )
    ...
    @patch("src.experiments.result_exporter.DependencyTracker")
    def test_validate_export_integrity(
        self, mock_tracker, temp_output_dir, mock_experiment
    ):
    ...
    def test_compress_export_zip(self, mock_tracker, temp_output_dir, mock_experiment):
        ...
        archive_path = exporter.compress_export(
            export_dir, compression_type=CompressionType.ZIP
        )
    ...
    def test_cleanup_old_exports(self, mock_tracker, temp_output_dir, mock_experiment):
        ...
        cleanup_result = exporter.cleanup_old_exports(keep_count=10, keep_days=30)
    ...
    def test_generate_summary_report(
        ...
        report_path = exporter.generate_summary_report(
            export_dirs, include_statistics=True, include_trends=False
        )
    ...
    def test_unsupported_format_error(self, temp_output_dir, mock_experiment):
        ...
        with patch.object(exporter, "_export_to_format") as mock_export:
            ...
            result = exporter.export_experiment(
                mock_experiment, formats=["unsupported_format"]
            )
...
class TestExportFormats:
    ...
    def test_hdf5_format(
        self, mock_h5py, mock_tracker, temp_output_dir, mock_experiment
    ):
        """Тест HDF5 экспорта."""
        # Мокаем h5py.File
        mock_file = Mock()
        ...
        result = exporter.export_experiment(
            mock_experiment, formats=[ExportFormat.HDF5]
        )
    ...
    def test_pickle_format(self, mock_tracker, temp_output_dir, mock_experiment):
        ...
        result = exporter.export_experiment(
            mock_experiment, formats=[ExportFormat.PICKLE]
        )
...
class TestCompressionTypes:
    ...
    def test_tar_gz_compression(self, mock_tracker, temp_output_dir, mock_experiment):
        ...
        export_dir = Path(result["export_dir"])
        archive_path = exporter.compress_export(
            export_dir, compression_type=CompressionType.TAR_GZ
        )
...
class TestUtilityFunctions:
    ...
    def test_export_experiment_results_function(
        self, mock_exporter_class, mock_experiment
    ):
        ...
        mock_exporter_class.assert_called_once_with(
            output_dir="/test/dir", include_dependencies=True
        )

        # Проверяем, что метод экспорта был вызван
        mock_exporter.export_experiment.assert_called_once_with(
            mock_experiment, [ExportFormat.JSON]
        )

        assert result == {"test": "result"}

    @patch("src.experiments.result_exporter.ResultExporter")
    def test_export_multiple_experiments_results_function(
        self, mock_exporter_class, mock_experiment
    ):
        ...
        mock_exporter.export_multiple_experiments.assert_called_once_with(
            experiments, [ExportFormat.JSON], include_comparison=True
        )
...
class TestDataFlattening:
    ...
    def test_flatten_data_for_csv(self, temp_output_dir):
        ...
        exporter = ResultExporter(
            output_dir=temp_output_dir, include_dependencies=False
        )
    ...
    def test_flatten_data_without_metrics(self, temp_output_dir):
        ...
        test_data = {
            "experiment_id": "test_001",
            "status": "completed",
            "nested": {"value": 42, "deep": {"nested": "test"}},
        }
