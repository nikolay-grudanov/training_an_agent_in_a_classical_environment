...
class TestDependencyTracker:
    ...
    def test_get_conda_packages(self, mock_run, tracker):
        ...
        mock_run.return_value = MagicMock(
            stdout=json.dumps(mock_packages),
            returncode=0
        )
    ...
    def test_create_dependency_snapshot(self, mock_json_dump, mock_file, tracker):
        ...
        with patch.object(tracker, 'get_system_info') as mock_system, \
    ...
    def test_detect_dependency_conflicts_gym_gymnasium(self, tracker):
        ...
        with patch.object(tracker, 'get_pip_packages') as mock_packages:
            ...
            gym_conflict = next((c for c in conflicts if c['type'] == 'library_conflict'), None)
    ...
    def test_generate_compatibility_report(self, tracker):
        ...
        mock_snapshot = {
            ...
            'ml_libraries': {'numpy': '1.21.0', 'stable-baselines3': '1.5.0', 'gymnasium': '0.26.0'}
        }
        
        with patch.object(tracker, 'create_dependency_snapshot') as mock_create, \
...
class TestUtilityFunctions:
    ...
    def test_validate_environment_for_experiment_success(self, mock_tracker_class):
        ...
        mock_tracker.validate_reproducibility.assert_called_once_with("reference_snapshot")
...
class TestDependencyTrackerIntegration:
    ...
    def test_real_package_detection(self, tmp_path):
        ...
        assert len(found_packages) > 0, f"Не найдено ни одного из ожидаемых пакетов: {common_packages}"
