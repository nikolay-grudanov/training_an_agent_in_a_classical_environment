...
class TestQuantitativeEvaluator:
    ...
    def test_evaluate_agent_quantitative_basic(
        self, quantitative_evaluator, stable_agent
    ):
    ...
    def test_evaluate_agent_quantitative_invalid_episodes(
        self, quantitative_evaluator, stable_agent
    ):
    ...
    def test_consecutive_success_rate_calculation(self, quantitative_evaluator):
        ...
        # Все успешные
        assert (
            quantitative_evaluator._calculate_consecutive_success_rate(
                [True, True, True]
            )
            == 1.0
        )
        ...
        result = quantitative_evaluator._calculate_consecutive_success_rate(
    ...
    def test_cohens_d_calculation(self, quantitative_evaluator):
        ...
        d_zero_std = quantitative_evaluator._calculate_cohens_d(
            [100.0] * 5, [100.0] * 5
        )
        assert d_zero_std == 0.0

    def test_compare_with_baseline_agent(
        self, quantitative_evaluator, stable_agent, variable_agent
    ):
        ...
        with patch.object(
            ...
            variable_metrics = QuantitativeMetrics(
                ...
                length_iqr=0.0,
                reward_cv=0.167,
                reward_stability_score=0.833,
                consecutive_success_rate=0.7,
                reward_trend_slope=0.0,
                learning_efficiency=12.0,
                outlier_count=0,
            )
    ...
    def test_compare_with_baseline_metrics(
        self, quantitative_evaluator, variable_agent
    ):
        ...
        baseline_base = EvaluationMetrics(
            ...
            min_episode_length=90,
            max_episode_length=110,
            reward_ci_lower=75.0,
            reward_ci_upper=85.0,
            success_rate=0.6,
            total_episodes=10,
            total_timesteps=1000,
            evaluation_time=10.0,
            episode_rewards=[80.0] * 10,
            episode_lengths=[100] * 10,
        ...
        baseline_metrics = QuantitativeMetrics(
            ...
            length_iqr=0.0,
            reward_cv=0.125,
            reward_stability_score=0.875,
            consecutive_success_rate=0.5,
            reward_trend_slope=0.0,
            learning_efficiency=8.0,
            outlier_count=0,
        )
        ...
        with patch.object(
            ...
            agent_base = EvaluationMetrics(
                ...
                max_reward=130.0,
                mean_episode_length=100.0,
                std_episode_length=8.0,
                min_episode_length=85,
                max_episode_length=115,
                reward_ci_lower=90.0,
                reward_ci_upper=110.0,
                success_rate=0.8,
                total_episodes=10,
                total_timesteps=1000,
                evaluation_time=10.0,
    ...
    def test_compare_with_baseline_no_baseline(
        self, quantitative_evaluator, variable_agent
    ):
        """Тест сравнения без предоставления базовой линии."""
        with pytest.raises(
            ValueError, match="Необходимо предоставить либо baseline_agent"
        ):
        ...
        with patch.object(
            ...
            def create_mock_metrics(mean_reward):
                base = EvaluationMetrics(
                    ...
                    std_reward=5.0,
                    min_reward=mean_reward - 10,
                    max_reward=mean_reward + 10,
                    mean_episode_length=100.0,
                    std_episode_length=5.0,
                    min_episode_length=95,
                    max_episode_length=105,
                    reward_ci_lower=mean_reward - 5,
                    reward_ci_upper=mean_reward + 5,
                    success_rate=0.8,
                    total_episodes=10,
                    total_timesteps=1000,
                    evaluation_time=10.0,
                    episode_rewards=[mean_reward] * 10,
                    episode_lengths=[100] * 10,
                    episode_successes=[True] * 8 + [False] * 2,
                )
                return QuantitativeMetrics(
                    base_metrics=base,
                    reward_median=mean_reward,
                    reward_q25=mean_reward - 5,
                    reward_q75=mean_reward + 5,
                    reward_iqr=10.0,
                    reward_skewness=0.0,
                    reward_kurtosis=0.0,
                    length_median=100.0,
                    ...
                    length_iqr=10.0,
                    reward_cv=0.05,
                    reward_stability_score=0.95,
                    consecutive_success_rate=0.7,
                    reward_trend_slope=0.0,
                    learning_efficiency=mean_reward / 10,
                    outlier_count=0,
                )
            ...
            with patch.object(
                quantitative_evaluator.evaluator, "compare_agents"
            ) as mock_compare:
                ...
                mock_compare.return_value = ComparisonResult(
                    agent1_name="agent1",
                    agent2_name="agent2",
                    reward_ttest_statistic=2.0,
                    reward_ttest_pvalue=0.05,
                    reward_significant=True,
                    length_ttest_statistic=1.0,
                    length_ttest_pvalue=0.3,
                    length_significant=False,
                    reward_effect_size=0.8,
                    length_effect_size=0.2,
                    better_agent="agent2",
    ...
    def test_visualize_results_single_agent(self, quantitative_evaluator):
        ...
        with patch("matplotlib.pyplot.show") as mock_show:
            quantitative_evaluator.visualize_results(metrics=metrics, show_plots=False)
...
class TestConvenienceFunctions:
    ...
    def test_evaluate_agent_standard(self, mock_env):
        ...
        with patch(
            "src.evaluation.quantitative_eval.QuantitativeEvaluator"
        ...
        with patch(
            ...
            def create_mock_metrics(mean_reward):
                base_metrics = EvaluationMetrics(
                    ...
                    total_timesteps=2000,
                    evaluation_time=20.0,
                    episode_rewards=[mean_reward] * 20,
                    episode_lengths=[100] * 20,
                    episode_successes=[True] * 20,
                )
                ...
                return QuantitativeMetrics(
                    ...
                    consecutive_success_rate=1.0,
                    reward_trend_slope=0.0,
                    learning_efficiency=mean_reward / 20,
                    outlier_count=0,
                )
