...
class TestPPOAgent:
    ...
    @patch("src.agents.ppo_agent.get_experiment_logger")
    @patch("src.agents.ppo_agent.get_metrics_tracker")
    ...
    def test_learning_rate_schedule_linear(
        ...
        assert final_lr == config.learning_rate * config.lr_final_ratio
...
def test_reproducibility_with_different_seeds(seed: int) -> None:
    ...
    with patch("src.agents.ppo_agent.make_vec_env") as mock_make_vec_env, \
         patch("src.agents.ppo_agent.get_experiment_logger"), \
         patch("src.agents.ppo_agent.get_metrics_tracker"):
