...
from src.utils.config import (
    RLConfig, AlgorithmConfig, ReproducibilityConfig,
    ConfigLoader, load_config_with_seeds, enforce_global_seed_consistency
)
...
class TestRLConfigSeedMethods:
    ...
    def test_validate_reproducibility_no_conflicts(self):
        ...
        config = RLConfig(
            seed=42,
            algorithm=AlgorithmConfig(name="PPO", seed=42, device="cpu"),  # Явно указываем device
            reproducibility=ReproducibilityConfig(seed=42, deterministic=True, use_cuda=False)
        )
    ...
    def test_apply_seeds_success(self, mock_verify, mock_set_seed):
        ...
        config = RLConfig(
            seed=42,
            reproducibility=ReproducibilityConfig(validate_determinism=True)
        )
    ...
    def test_apply_seeds_reproducibility_failure(self, mock_verify, mock_set_seed):
        ...
        with pytest.raises(RuntimeError, match="Не удалось обеспечить воспроизводимость"):
            config.apply_seeds()
    ...
    def test_load_config_with_seeds(self, mock_verify, mock_set_seed):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('src.utils.config.get_config_loader') as mock_get_loader:
                ...
                mock_loader.load_config_with_seed_validation.return_value = RLConfig(seed=42)
...
class TestIntegrationWithSeeding:
    ...
    def test_integration_without_validation(self, mock_set_seed):
        ...
        config = RLConfig(
            seed=42,
            reproducibility=ReproducibilityConfig(validate_determinism=False)
        )
