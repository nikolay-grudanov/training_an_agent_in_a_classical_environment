...
class TestRecordAgentEpisode:
    ...
    def test_record_episode_with_string_env(self, mock_agent, video_config, temp_dir):
        ...
        with patch(
            "src.visualization.video_generator.setup_recording_environment"
        ) as mock_setup:
...
class TestRecordMultipleEpisodes:
    ...
    def test_record_multiple_episodes_success(
        self, mock_agent, mock_env, video_config, temp_dir
    ):
        """Тест успешной записи нескольких эпизодов."""
        with patch(
            "src.visualization.video_generator.record_agent_episode"
        ) as mock_record:
        ...
        assert all(
            result["episode_number"] == i + 1 for i, result in enumerate(results)
        )
        assert mock_record.call_count == 3

    def test_record_multiple_episodes_with_seeds(
        self, mock_agent, mock_env, video_config, temp_dir
    ):
    ...
    def test_record_multiple_episodes_partial_failure(
        self, mock_agent, mock_env, video_config, temp_dir
    ):
...
class TestCreateTrainingMontage:
    ...
    def test_create_montage_no_frames(self, video_config, temp_dir):
        ...
        with patch(
            ...
            with pytest.raises(
                VideoGenerationError, match="Не удалось создать кадры для монтажа"
            ):
                create_training_montage(
                    agent_checkpoints=checkpoints,
                    env="CartPole-v1",
...
class TestGenerateComparisonVideo:
    ...
    def test_comparison_video_success(self, video_config, temp_dir):
        ...
        with patch(
            ...
            with patch("src.visualization.video_generator._save_video") as mock_save:
                with patch(
                    "src.visualization.video_generator._combine_frames_side_by_side"
                ) as mock_combine:
    ...
    def test_comparison_video_insufficient_agents(self, video_config, temp_dir):
        ...
        with pytest.raises(
            VideoGenerationError, match="Для сравнения нужно минимум 2 агента"
        ):
            generate_comparison_video(
                agents=agents,
                env="CartPole-v1",
...
class TestCompressVideo:
    ...
    def test_compress_video_success(self, mock_writer, mock_reader, temp_dir):
        ...
        mock_reader_instance.__iter__ = Mock(
            return_value=iter(
...
class TestUtilityFunctions:
    ...
    def test_combine_frames_side_by_side_multiple_frames(self):
        ...
        frames = [np.ones((50, 50, 3), dtype=np.uint8) * i * 50 for i in range(4)]
