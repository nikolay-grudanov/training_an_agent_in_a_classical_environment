...
class TestPerformancePlotter:
    ...
    def test_plot_reward_curve(self, mock_close, mock_savefig, plotter, sample_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result_path = plotter.plot_reward_curve(
                sample_data, y_col="value", save_path=save_path, smooth_window=10
            )
    ...
    def test_plot_episode_lengths(self, mock_close, mock_savefig, plotter, sample_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result_path = plotter.plot_episode_lengths(
                sample_data, x_col="episode", y_col="value", save_path=save_path
            )
    ...
    def test_plot_multiple_agents(self, mock_close, mock_savefig, plotter, sample_data):
        ...
        agents_data = {"agent1": sample_data.copy(), "agent2": sample_data.copy()}
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result_path = plotter.plot_multiple_agents(
                agents_data, metric="value", save_path=save_path
            )
    ...
    def test_create_dashboard(self, mock_close, mock_savefig, plotter):
        ...
        data = {
            "episode_reward": pd.DataFrame(
                {"timestep": range(1, 51), "value": np.random.normal(10, 2, 50)}
            ),
            "episode_length": pd.DataFrame(
                {"timestep": range(1, 51), "value": np.random.normal(100, 10, 50)}
            ),
        }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result_path = plotter.create_dashboard(data, save_path=save_path)
...
class TestInteractivePlotter:
    ...
    def sample_data(self):
        """Создает тестовые данные."""
        return pd.DataFrame(
            {"timestep": range(1, 101), "value": np.random.normal(10, 2, 100)}
        )
    ...
    def test_plot_interactive_reward_curve(
        self, mock_write_html, interactive_plotter, sample_data
    ):
    ...
    @patch("plotly.graph_objects.Figure.write_html")
    def test_plot_interactive_comparison(
        self, mock_write_html, interactive_plotter, sample_data
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result_path = interactive_plotter.plot_interactive_comparison(
                agents_data, save_path=save_path
            )
...
class TestUtilityFunctions:
    ...
    def test_export_plots_to_formats(self, mock_plot_function):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            saved_files = export_plots_to_formats(
                mock_plot_function, temp_dir, formats=["png", "pdf"], title="Test Plot"
            )
    ...
    def test_create_performance_report(self, mock_interactive, mock_static):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result_path = create_performance_report(data, output_dir=temp_dir)
    ...
    def test_quick_reward_plot_from_csv(self, mock_plot, mock_load):
        ...
        mock_load.return_value = pd.DataFrame(
            {"timestep": range(1, 11), "value": range(1, 11)}
        )
    ...
    @patch("src.visualization.performance_plots.DataLoader.load_from_csv")
    @patch(
        "src.visualization.performance_plots.PerformancePlotter.plot_multiple_agents"
    )
    def test_quick_comparison_plot(self, mock_plot, mock_load):
        ...
        agents_data = {"agent1": "agent1.csv", "agent2": "agent2.csv"}
...
class TestIntegration:
    ...
    def test_full_workflow_with_metrics_tracker(self):
        ...
        for i in range(100):
            tracker.add_metric(
                "episode_reward", np.random.normal(10, 2), timestep=i, episode=i // 10
            )
            tracker.add_metric(
                "episode_length",
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            dashboard_path = plotter.create_dashboard(
                data, save_path=Path(temp_dir) / "dashboard.png"
            )
    ...
    def test_performance_report_creation(
        self, mock_write_html, mock_close, mock_savefig
    ):
