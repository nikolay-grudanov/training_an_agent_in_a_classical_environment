...
class TestBestEpisodeDemo:
    ...
    def test_create_best_episode_demo_success(
        ...
        with (
            patch("numpy.random.randint", side_effect=[42, 43, 44]),
            ...
            def step_side_effect(*args):
                reward = episode_rewards[
                    mock_env.step.call_count % len(episode_rewards)
                ]
                return (
    ...
    def test_create_best_episode_demo_no_episodes(
        self, mock_setup_env, mock_env, demo_config, temp_output_dir
    ):
    ...
    def test_create_best_episode_demo_auto_naming(self, mock_env, temp_output_dir):
        ...
        with (
            patch("src.visualization.agent_demo.record_agent_episode") as mock_record,
            patch(
                "src.visualization.agent_demo.setup_recording_environment"
            ) as mock_setup,
            patch.object(agent, "predict", return_value=(1, None)),
        ):
...
class TestAverageBehaviorDemo:
    ...
    def test_create_average_behavior_demo_success(
        self, mock_rmtree, mock_copy, mock_record_multiple, demo_config, temp_output_dir
    ):
    ...
    def test_create_average_behavior_demo_no_successful_episodes(
        self, mock_record_multiple, demo_config, temp_output_dir
    ):
        ...
        with pytest.raises(
            AgentDemoError, match="Не удалось записать ни одного успешного эпизода"
        ):
            create_average_behavior_demo(
                agent=agent,
                env="CartPole-v1",
...
class TestBeforeAfterDemo:
    ...
    def test_create_before_after_demo_success(
        self, mock_generate_comparison, demo_config, temp_output_dir
    ):
...
class TestTrainingProgressDemo:
    ...
    def test_create_training_progress_demo_success(
        self, mock_create_montage, demo_config, temp_output_dir
    ):
    ...
    def test_create_training_progress_demo_load_error(
        self, demo_config, temp_output_dir
    ):
        ...
        with patch.object(
            MockAgent, "load", side_effect=FileNotFoundError("Файл не найден")
        ):
...
class TestBatchDemos:
    ...
    def test_create_batch_demos_success(
        self, mock_create_average, mock_create_best, demo_config, temp_output_dir
    ):
    ...
    def test_create_batch_demos_with_errors(
        self, mock_create_best, demo_config, temp_output_dir
    ):
