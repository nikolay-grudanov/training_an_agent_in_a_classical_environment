...
def sample_experiments():
    ...
    exp1.add_result("baseline", {
        "mean_reward": 150.0,
        "final_reward": 180.0,
        "training_time": 3600.0,
        "metrics_history": [
            {"episode_reward": 100 + i * 2, "timestep": i * 1000}
            for i in range(50)
        ]
    })
    
    exp2.add_result("baseline", {
        "mean_reward": 120.0,
        "final_reward": 140.0,
...
class TestExperimentComparator:
    ...
    def test_statistical_significance_t_test(self, comparator):
        ...
        assert isinstance(result.significant, (bool, np.bool_))  # Принимаем и bool и numpy.bool_
    ...
    def test_learning_efficiency(self, comparator, sample_experiments):
        ...
        efficiency = comparator.learning_efficiency(
            sample_experiments, threshold=150.0
        )
    ...
    def test_compare_experiments_insufficient_experiments(self, comparator):
        ...
        with pytest.raises(ValueError, match="Для сравнения необходимо минимум 2 эксперимента"):
...
class TestEdgeCases:
    ...
    def test_missing_hyperparameter(self, comparator, sample_experiments):
        ...
        with pytest.raises(ValueError, match="Недостаточно данных для анализа чувствительности"):
