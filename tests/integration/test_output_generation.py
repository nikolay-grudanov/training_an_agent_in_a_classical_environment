...
class MockAgent:
    ...
    def predict(self, observation, deterministic: bool = True, **kwargs) -> Tuple[np.ndarray, Any]:
...
class MockEnvironment:
    ...
    def reset(self, seed: Optional[int] = None):
        ...
        if seed is not None:
            np.random.seed(seed)
        self._episode_length = 0
        observation = np.random.random(8)  # LunarLander имеет 8-мерное пространство наблюдений
        info = {}
...
class TestOutputGeneration:
    ...
    def training_data(self) -> Dict[str, pd.DataFrame]:
        ...
        ppo_rewards = 100 * (1 - np.exp(-timesteps / 3000)) + np.random.normal(0, 10, len(timesteps))
        ppo_lengths = 200 - 50 * (1 - np.exp(-timesteps / 2000)) + np.random.normal(0, 5, len(timesteps))
        ...
        a2c_rewards = 80 * (1 - np.exp(-timesteps / 4000)) + np.random.normal(0, 15, len(timesteps))
        a2c_lengths = 220 - 40 * (1 - np.exp(-timesteps / 3000)) + np.random.normal(0, 8, len(timesteps))
        ...
        return {
            ...
            'episode_length': ppo_data[['timestep', 'episode', 'episode_length', 'timestamp']].rename(
                columns={'episode_length': 'value'}
            )
        }
    ...
    def test_agent_demo_videos_generation(
        ...
        with patch('src.visualization.agent_demo.create_best_episode_demo') as mock_batch_demo:
        ...
        with patch('src.visualization.agent_demo.create_best_episode_demo') as mock_quick:
    ...
    def test_quantitative_evaluation(
        ...
        with patch.object(evaluator.evaluator, 'evaluate_agent') as mock_eval:
            ...
            for name, agent in trained_agents.items():
                ...
                print(f"✅ {name}: награда {metrics.base_metrics.mean_reward:.2f} ± "
            ...
            baseline_agent = trained_agents["SAC_Poor"]  # Используем слабого агента как базовую линию
            ...
            print(f"✅ Сравнение: улучшение {comparison.reward_improvement:.1f}%, "
    ...
    def test_results_formatting_and_reporting(
        self,
        test_output_dir: Path
    ):
    ...
    def test_integration_between_modules(
        ...
        interactive_plots = list(Path(performance_report_dir).glob("interactive/*.html"))
        
        assert len(static_plots) >= 2
        assert len(interactive_plots) >= 1
        print(f"✅ Отчет о производительности: {len(static_plots)} статических, "
        ...
        with patch('src.evaluation.evaluator.Evaluator.evaluate_agent') as mock_eval:
            ...
            formatter = ResultsFormatter(output_dir=integration_dir / "integrated_reports")
    ...
    def test_error_handling_and_edge_cases(
        ...
        with pytest.raises(ValueError, match="не найдена"):
            invalid_data = {"nonexistent_metric": pd.DataFrame({"x": [1, 2], "y": [1, 2]})}
    ...
    def test_performance_and_timing(
        ...
        print(f"✅ Оценка: {agents_evaluated} агентов за {evaluation_time:.2f}с "
        ...
        test_metrics = EvaluationMetrics(
            ...
            mean_episode_length=200.0, std_episode_length=20.0, min_episode_length=150, max_episode_length=250,
            success_rate=0.8, total_episodes=20, total_timesteps=4000, evaluation_time=30.0,
            episode_rewards=[150.0] * 20, episode_lengths=[200] * 20, episode_successes=[True] * 16 + [False] * 4,
        ...
        performance_metrics["reports"] = {
            ...
            "reports_per_second": reports_created / reports_time if reports_time > 0 else 0
        }
        ...
        assert performance_metrics["plots"]["time_per_plot"] < 5.0  # Не более 5 секунд на график
        assert performance_metrics["evaluation"]["time_per_agent"] < 2.0  # Не более 2 секунд на агента
        assert performance_metrics["reports"]["time_per_report"] < 3.0  # Не более 3 секунд на отчет
    ...
    def test_full_output_generation_workflow(
        ...
        try:
            ...
            workflow_results["files_created"].extend([str(p) for p in interactive_plots])
            workflow_results["steps_completed"].append("performance_plots")
            
            print(f"✅ Создано графиков: {len(static_plots)} статических, {len(interactive_plots)} интерактивных")
            ...
            with patch('src.visualization.agent_demo.create_best_episode_demo') as mock_demo:
            ...
            experiment_data = {
                ...
                "evaluation_results": {
                    ...
                    "evaluation_time": batch_evaluation.statistical_summary["evaluation_time"]
                },
                "output_files": {
                    "plots_created": len(static_plots) + len(interactive_plots),
            ...
            for format_type in ["html", "markdown"]:
                report_path = formatter.format_experiment_report(
                    ...
                    experiment_data=experiment_data,
                    output_format=format_type,
                    filename=f"full_workflow_report"
                )
                workflow_results["files_created"].append(str(report_path))
            ...
            for file_path in workflow_results["files_created"]:
                ...
                if path_obj.exists():
                    ...
                    files_verification["file_sizes"][str(path_obj)] = path_obj.stat().st_size
                else:
        ...
        assert len(workflow_results["steps_completed"]) == 6, "Все шаги должны быть выполнены"
