...
class TestOutputGenerationSimple:
    ...
    def sample_training_data(self) -> Dict[str, pd.DataFrame]:
        ...
        ppo_rewards = 100 * (1 - np.exp(-timesteps / 2000)) + np.random.normal(
            0, 10, len(timesteps)
        )
    ...
    def test_performance_plots_creation(
        self, sample_training_data: Dict[str, pd.DataFrame], test_output_dir: Path
    ):
    ...
    def test_demo_videos_creation(self, test_output_dir: Path):
    ...
    def test_quantitative_evaluation(self, mock_evaluate_agent, test_output_dir: Path):
        ...
        def mock_evaluation(agent, num_episodes, **kwargs):
            ...
            return EvaluationMetrics(
                ...
                reward_ci_lower=float(
                    np.mean(rewards) - 1.96 * np.std(rewards) / np.sqrt(num_episodes)
                ),
        ...
        metrics = evaluator.evaluate_agent_quantitative(
            agent=mock_agent, num_episodes=15, agent_name="TestAgent"
        )
        ...
        print(
            f"✅ Оценка агента: награда {metrics.base_metrics.mean_reward:.2f} ± "
    ...
    def test_results_formatting(self, test_output_dir: Path):
        ...
        # 2. Экспорт в CSV
        csv_export = formatter.export_to_csv(data=test_metrics, filename="test_results")
        ...
        json_export = formatter.export_to_json(
            data={"test_metrics": test_metrics}, filename="test_results_json"
        )
    ...
    def test_performance_measurement(
        ...
        assert (
            performance_metrics["plots"]["time_per_plot"] < 10.0
        )  # Не более 10 секунд на график
        assert (
            performance_metrics["reports"]["time_per_report"] < 5.0
        )  # Не более 5 секунд на отчет
    ...
    def test_complete_output_generation_workflow(
        ...
        try:
            ...
            csv_export = formatter.export_to_csv(
                data=test_metrics, filename="workflow_results"
            )
        ...
        assert len(workflow_results["steps_completed"]) == 4, (
