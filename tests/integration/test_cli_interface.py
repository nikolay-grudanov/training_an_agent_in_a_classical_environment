...
class TestCLIInterface:
    ...
    def test_cli_help_command(self):
        ...
        try:
            ...
            assert result.returncode == 0, f"Команда помощи завершилась с ошибкой: {result.stderr}"
    ...
    def test_cli_validation_mode(self, test_config_path: Path, temp_output_dir: Path):
        ...
        try:
            ...
            if result.returncode == 0:
                assert "валидация" in result.stdout.lower() or "validation" in result.stdout.lower()
    ...
    def test_cli_argument_parsing(self):
        ...
        for args in test_cases:
            try:
                ...
                assert result.returncode == 0, f"Ошибка парсинга аргументов {args}: {result.stderr}"
    ...
    def test_cli_error_handling(self, temp_output_dir: Path):
        ...
        for test_case in error_test_cases:
            try:
                ...
                assert result.returncode != 0, f"Команда должна была завершиться с ошибкой для {test_case['args']}"
                ...
                if "не найден" in expected_error:
                    assert any(word in error_output for word in ["not found", "не найден", "error", "ошибка"])
                elif "invalid" in expected_error:
                    assert any(word in error_output for word in ["invalid", "неверный", "error", "ошибка"])
                
            except subprocess.TimeoutExpired:
                pytest.fail(f"Обработка ошибки для {test_case['args']} выполнялась слишком долго")
            except FileNotFoundError:
                pytest.skip("CLI модуль не найден или не настроен")

    def test_cli_output_directory_creation(self, test_config_path: Path, temp_output_dir: Path):
        ...
        for verbose_args in verbose_levels:
            try:
                result = subprocess.run([
                    ...
                    "--config", str(test_config_path),
                    "--mode", "validation",
                    "--output-dir", str(temp_output_dir / f"verbose_test_{len(verbose_args)}")
                ] + verbose_args,
                capture_output=True,
                text=True,
                timeout=60
                ...
                print(f"✅ Verbose уровень {len(verbose_args)}: код возврата {result.returncode}")
                ...
                pytest.fail(f"Verbose уровень {len(verbose_args)} выполнялся слишком долго")
            except FileNotFoundError:
                pytest.skip("CLI модуль не найден или не настроен")

    def test_cli_configuration_override(self, test_config_path: Path, temp_output_dir: Path):
        ...
        for params in parameter_combinations:
            try:
                result = subprocess.run([
                    "python", "-m", "src.experiments.runner",
                    "--config", str(test_config_path),
                    "--output-dir", str(temp_output_dir / f"override_test_{len(params)}")
                ] + params,
                capture_output=True,
                text=True,
                timeout=60
                ...
                pytest.fail(f"Переопределение параметров {params} выполнялось слишком долго")
            except FileNotFoundError:
                pytest.skip("CLI модуль не найден или не настроен")

    @pytest.mark.slow
    def test_cli_full_experiment_simulation(self, test_config_path: Path, temp_output_dir: Path):
    ...
    def test_cli_environment_variables(self, test_config_path: Path, temp_output_dir: Path):
