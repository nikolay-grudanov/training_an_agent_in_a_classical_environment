...
class MockReproducibleAgent:
    ...
    def train(self, env: Any, total_timesteps: int = 1000) -> Dict[str, Any]:
        ...
        return {
            'final_reward': float(np.mean(episode_rewards[-5:])),  # Среднее за последние 5 эпизодов
...
class MockReproducibleEnvironment:
    ...
    def reset(self) -> np.ndarray:
        ...
        self.state = np.random.randn(4).astype(np.float32)  # Детерминированное начальное состояние
        self.step_count = 0
        return self.state
...
class TestReproducibilityIntegration:
    ...
    def test_full_reproducibility_workflow(
        ...
        snapshot1 = dependency_tracker.create_dependency_snapshot(f"{experiment_id}_run1")
        ...
        snapshot2 = dependency_tracker.create_dependency_snapshot(f"{experiment_id}_run2")
        ...
        assert training_results1['episode_rewards'] == training_results2['episode_rewards']
        ...
        snapshot_comparison = dependency_tracker.compare_snapshots(
            snapshot1['metadata']['name'],
            snapshot2['metadata']['name']
        )
        ...
        assert len(changes['packages_added']) == 0, "Добавлены новые пакеты между запусками"
        ...
        assert validation_result['valid'], f"Экспорт не валиден: {validation_result['errors']}"
    ...
    def test_reproducibility_with_different_strictness_levels(
        ...
        for strictness in strictness_levels:
            ...
            report = checker.check_reproducibility(f"{experiment_id}_{strictness.value}")
        ...
        assert len(paranoid_report.recommendations) >= len(minimal_report.recommendations)
    ...
    def test_reproducibility_failure_detection(
        ...
        assert not report.is_reproducible, "Должно быть обнаружено нарушение воспроизводимости"
    ...
    def test_determinism_validation(
        ...
        assert det_result['is_deterministic'], "Детерминированная функция должна пройти тест"
    ...
    def test_reproducibility_with_performance_measurement(
        ...
        snapshot = dependency_tracker.create_dependency_snapshot(f"{experiment_id}_perf")
        ...
        assert snapshot_time < 10.0, f"Создание снимка слишком медленное: {snapshot_time}s"
        assert experiment_time < 5.0, f"Эксперимент слишком медленный: {experiment_time}s"
        assert registration_time < 2.0, f"Регистрация слишком медленная: {registration_time}s"
        assert check_time < 5.0, f"Проверка воспроизводимости слишком медленная: {check_time}s"
    ...
    def test_edge_cases_and_error_handling(
        ...
        def failing_training_function(seed: int) -> Dict[str, Any]:
            ...
            set_seed(seed)
            return {
                'final_reward': float(np.random.randn()),
                'success': True
            }
        ...
        assert not report.is_reproducible, "Тест с ошибками не должен быть воспроизводим"
        ...
        with patch.object(dependency_tracker, 'create_dependency_snapshot') as mock_snapshot:
            ...
            assert run_id is not None, "Регистрация должна работать даже при ошибках снимка"
        ...
        assert not validation_result['valid'], "Поврежденный экспорт должен быть невалидным"
    ...
    def test_comprehensive_reproducibility_report_generation(
        ...
        for i, run_config in enumerate(run_configs):
            agent = MockReproducibleAgent(run_config['agent_id'], seed=run_config['seed'])
        ...
        critical_issues = [
            issue for issue in diagnosis['issues_found'] 
            if issue.get('severity') == 'critical'
        ]
        assert len(critical_issues) == 0, f"Найдены критические проблемы: {critical_issues}"
