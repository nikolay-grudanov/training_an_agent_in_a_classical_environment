...
class TestExperimentRunner:
    ...
    def test_run_configuration_baseline(self, mock_trainer_class, mock_experiment, mock_training_result):
    ...
    @patch('src.experiments.runner.Trainer')
    def test_run_configuration_variant(self, mock_trainer_class, mock_experiment, mock_training_result):
    ...
    @patch('src.experiments.runner.Trainer')
    def test_run_configuration_training_failure(self, mock_trainer_class, mock_experiment):
    ...
    def test_run_sequential_success(self, mock_setup, mock_validate, mock_execute, mock_experiment):
        """Тест успешного выполнения в последовательном режиме."""
        # Настройка моков
        mock_execute.return_value = True
        
        runner = ExperimentRunner(mock_experiment, execution_mode=ExecutionMode.SEQUENTIAL)
    ...
    @patch('src.experiments.runner.ExperimentRunner._validate_experiment')
    @patch('src.experiments.runner.ExperimentRunner._setup_environment')
    def test_run_parallel_success(self, mock_setup, mock_validate, mock_execute, mock_experiment):
        """Тест успешного выполнения в параллельном режиме."""
        # Настройка моков
        mock_execute.return_value = True
        
        runner = ExperimentRunner(mock_experiment, execution_mode=ExecutionMode.PARALLEL)
    ...
    @patch('src.experiments.runner.ExperimentRunner._validate_experiment')
    @patch('src.experiments.runner.ExperimentRunner._setup_environment')
    def test_run_validation_mode(self, mock_setup, mock_validate, mock_validate_configs, mock_experiment):
        """Тест выполнения в режиме валидации."""
        # Настройка моков
        mock_validate_configs.return_value = True
        
        runner = ExperimentRunner(mock_experiment, execution_mode=ExecutionMode.VALIDATION)
    ...
    def test_run_keyboard_interrupt(self, mock_experiment):
        ...
        # Имитация прерывания
        with patch.object(runner, '_validate_experiment', side_effect=KeyboardInterrupt):
            result = runner.run()
    ...
    def test_create_trainer_config(self, mock_trainer_class, mock_experiment):
        ...
        config = runner._create_trainer_config(
            mock_experiment.baseline_config,
            "baseline"
        )
        ...
        assert config.environment_name == mock_experiment.baseline_config.environment.name
        assert config.total_timesteps == mock_experiment.baseline_config.training.total_timesteps
...
class TestExperimentRunnerIntegration:
    ...
    @patch('src.experiments.runner.Trainer')
    def test_full_sequential_execution(self, mock_trainer_class, mock_experiment, mock_training_result):
    ...
    @patch('src.experiments.runner.Trainer')
    def test_full_parallel_execution(self, mock_trainer_class, mock_executor_class, mock_experiment, mock_training_result):
        ...
        with patch('src.experiments.runner.as_completed', return_value=[mock_future, mock_future]):
            runner = ExperimentRunner(mock_experiment, execution_mode=ExecutionMode.PARALLEL)
            
            # Имитация результатов
            results = [("baseline", mock_training_result), ("variant", mock_training_result)]
