...
class TrainingStatistics:
    ...
    def update_from_progress(self, progress: TrainingProgress) -> None:
        ...
        self.peak_memory_usage_mb = max(self.peak_memory_usage_mb, progress.memory_usage_mb)
...
class TrainingHook(Protocol):
    ...
    def on_episode_end(self, progress: TrainingProgress, episode_info: Dict[str, Any]) -> None:
        """Вызывается в конце эпизода."""
        ...
    
    def on_training_end(self, progress: TrainingProgress, statistics: TrainingStatistics) -> None:
...
class ProgressReporter:
    ...
    def _print_console_progress(self, progress: TrainingProgress) -> None:
        ...
        print(f"\r[{bar}] {progress_pct:.1f}% | "
              ...
              f"Mem: {progress.memory_usage_mb:.0f}MB", end="")
    ...
    def _log_tensorboard(self, progress: TrainingProgress) -> None:
        ...
        self.tb_writer.add_scalar("Training/MeanReward", progress.mean_episode_reward, step)
        self.tb_writer.add_scalar("Training/StdReward", progress.std_episode_reward, step)
        self.tb_writer.add_scalar("Training/BestReward", progress.best_episode_reward, step)
        
        # Производительность
        self.tb_writer.add_scalar("Performance/StepsPerSecond", progress.steps_per_second, step)
        self.tb_writer.add_scalar("Performance/EpisodesPerMinute", progress.episodes_per_minute, step)
        
        # Ресурсы
        self.tb_writer.add_scalar("Resources/MemoryMB", progress.memory_usage_mb, step)
        self.tb_writer.add_scalar("Resources/CPUPercent", progress.cpu_usage_percent, step)
        if progress.gpu_memory_mb > 0:
            self.tb_writer.add_scalar("Resources/GPUMemoryMB", progress.gpu_memory_mb, step)
        
        # Прогресс
        self.tb_writer.add_scalar("Progress/Percentage", progress.get_progress_percentage(), step)
...
class TrainingLoop:
    ...
    def _run_episodic_training(self) -> None:
        ...
        while (self.max_episodes is None or episode < self.max_episodes) and \
              total_timesteps < self.total_timesteps:
            ...
            if not self.interrupted:
                self._handle_episode_end(episode_reward, episode_length, total_timesteps)
    ...
    def _run_adaptive_training(self) -> None:
        ...
        while self.progress.current_timestep < self.total_timesteps:
            ...
            if (self.progress.current_timestep - last_switch_timestep) >= switch_interval:
    ...
    def _run_episodic_training_batch(self, max_timesteps: int) -> None:
        ...
        while (self.progress.current_timestep - start_timestep) < max_timesteps and \
              self.progress.current_timestep < self.total_timesteps:
            ...
            while not done and self.progress.current_timestep < self.total_timesteps:
                ...
                self.metrics_tracker.add_metric("step_reward", reward, self.progress.current_timestep)
                
                # Периодические операции
                self._handle_periodic_operations(self.progress.current_timestep)
            
            if not self.interrupted:
                self._handle_episode_end(episode_reward, episode_length, self.progress.current_timestep)
    ...
    def _handle_episode_end(self, episode_reward: float, episode_length: int, timestep: int) -> None:
    ...
    def _should_stop_due_to_resources(self) -> bool:
        ...
        if self.memory_limit_mb and self.progress.memory_usage_mb > self.memory_limit_mb:
            self.logger.warning(
...
class LoggingHook:
    ...
    def on_step(self, progress: TrainingProgress, step_info: Dict[str, Any]) -> None:
        if progress.current_timestep % self.log_interval == 0:
            self.logger.debug(
                f"Шаг {progress.current_timestep}",
                extra={"step_info": step_info}
            )
    
    def on_episode_end(self, progress: TrainingProgress, episode_info: Dict[str, Any]) -> None:
        self.logger.debug(
            f"Конец эпизода {episode_info['episode']}",
            extra=episode_info
        )
