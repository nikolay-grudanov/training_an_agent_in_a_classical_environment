...
def train(
    ...
    try:
        ...
        if result.success:
            ...
            console.print(
                f"❌ [bold red]Ошибка обучения: {result.error_message}[/bold red]"
            )
            sys.exit(1)
...
def resume(
    ...
    try:
        ...
        if result.success:
            console.print(
                "✅ [bold green]Восстановление завершено успешно![/bold green]"
            )
        else:
            console.print(
                f"❌ [bold red]Ошибка восстановления: {result.error_message}[/bold red]"
            )
            sys.exit(1)
...
def evaluate(
    ...
    deterministic: bool = typer.Option(
        True, "--deterministic", "-d", help="Детерминистическая политика"
    ),
    verbose: int = typer.Option(1, "--verbose", "-v", help="Уровень детализации"),
) -> None:
...
def compare(
    ...
    env: str = typer.Option(
        "LunarLander-v3", "--env", "-e", help="Среда для тестирования"
    ),
    ...
    seeds: List[int] = typer.Option(
        [42], "--seed", "-s", help="Seeds для экспериментов"
    ),
    output_dir: str = typer.Option(
        "results/comparison", "--output", "-o", help="Директория результатов"
    ),
) -> None:
        ...
        with Progress(console=console) as progress:
            ...
            for algorithm in algorithms:
                ...
                for run, seed in enumerate(seeds):
                    console.print(
                        f"🎯 {algorithm} - запуск {run + 1}/{runs} (seed={seed})"
                    )
...
def config(
    ...
    config_dir: str = typer.Option(
        "configs", "--dir", "-d", help="Директория конфигураций"
    ),
) -> None:
