...
class LunarLanderEnvironment(EnvironmentWrapper):
    ...
    def _validate_env_id(self, env_id: str) -> None:
        ...
        """
        if (
            env_id not in self.SUPPORTED_VERSIONS
            and env_id not in self.CONTINUOUS_ACTION_SPACE
        ):
            raise ValueError(
        ...
        info.update(
            {
                ...
                "reward_components": self._analyze_reward_components(
    ...
    def _determine_episode_outcome(self, info: Dict[str, Any]) -> None:
        ...
        if not self._current_landing_state["is_landing"]:
            ...
            is_successful = (
                ...
                and velocity <= self.LANDING_SUCCESS_VELOCITY_THRESHOLD
                and angle is not None
                and angle <= self.LANDING_SUCCESS_ANGLE_THRESHOLD
                and position is not None
                and position <= self.LANDING_SUCCESS_POSITION_THRESHOLD
            )
