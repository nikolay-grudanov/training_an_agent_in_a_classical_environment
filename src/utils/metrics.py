...
class MetricsTracker:
    ...
    def register_custom_metric(
        self,
        name: str,
        calculator: Callable[[List[MetricPoint]], float]
    ) -> None:
    ...
    def register_aggregator(
        self,
        metric_name: str,
        aggregator: Callable[[MetricPoint], None]
    ) -> None:
    ...
    def calculate_custom_metrics(self) -> Dict[str, float]:
        ...
        for name, calculator in self.custom_metrics.items():
            try:
                ...
                logger.error(f"Ошибка при вычислении пользовательской метрики {name}: {e}")
    ...
    def plot_metric(
        ...
        try:
            ...
            if show_moving_average and len(values) >= ma_window:
                ...
                plt.plot(timesteps, ma_values, linewidth=2, 
...
def sample_efficiency_metric(points: List[MetricPoint]) -> float:
    ...
    total_timesteps = max(p.timestep for p in reward_points) - min(p.timestep for p in reward_points)
...
def get_metrics_tracker(experiment_id: Optional[str] = None) -> MetricsTracker:
    ...
    if _global_tracker is None or (experiment_id and _global_tracker.experiment_id != experiment_id):
