...
class ExperimentConfig:
    ...
    metrics: List[str] = field(default_factory=lambda: ["mean_reward", "episode_length"])
...
class RLConfig:
    ...
    algorithm: AlgorithmConfig = field(default_factory=lambda: AlgorithmConfig(name="PPO"))
    environment: EnvironmentConfig = field(default_factory=lambda: EnvironmentConfig(name="LunarLander-v3"))
    ...
    def enforce_seed_consistency(self) -> None:
        ...
        if self.reproducibility.seed != self.seed:
            logger.debug(f"Обновление seed в reproducibility: {self.reproducibility.seed} -> {self.seed}")
            self.reproducibility.seed = self.seed
        
        # Синхронизируем seed в алгоритме
        if self.algorithm.seed != self.seed:
            logger.debug(f"Обновление seed в algorithm: {self.algorithm.seed} -> {self.seed}")
        ...
        if self.experiment and self.experiment.seeds:
            if self.seed not in self.experiment.seeds:
                logger.debug(f"Добавление основного seed в experiment.seeds: {self.seed}")
                if self.seed not in self.experiment.seeds:
    ...
    def validate_reproducibility(self) -> Tuple[bool, List[str]]:
        ...
        if warnings_list:
            logger.warning(f"Обнаружено {len(warnings_list)} предупреждений о воспроизводимости")
        else:
    ...
    def get_reproducibility_report(self) -> Dict[str, Any]:
        ...
        logger.info(f"Отчет о воспроизводимости сгенерирован. Статус: {'✓' if is_valid else '✗'}")
        return report
    ...
    def _check_cuda_settings(self) -> List[str]:
        ...
        try:
            ...
            if self.reproducibility.use_cuda and not torch.cuda.is_available():
                warnings_list.append(
                    "use_cuda=True, но CUDA недоступна в системе"
                )
    ...
    def _check_algorithm_reproducibility(self) -> List[str]:
        ...
        if self.algorithm.policy_kwargs:
            if "activation_fn" in self.algorithm.policy_kwargs:
                ...
                if hasattr(activation, "__name__") and "dropout" in activation.__name__.lower():
                    warnings_list.append(
    ...
    def _get_reproducibility_recommendations(self, warnings_list: List[str]) -> List[str]:
        ...
        if any("deterministic" in w.lower() and "benchmark" in w.lower() for w in warnings_list):
            recommendations.append(
...
class ConfigLoader:
    ...
    def merge_configs(self, base_config: RLConfig, override_config: Dict[str, Any]) -> RLConfig:
    ...
    def _apply_overrides(self, config_dict: Dict[str, Any], overrides: List[str]) -> Dict[str, Any]:
    ...
    def _validate_config(self, config: RLConfig) -> None:
        ...
        try:
            ...
            raise ValueError(f"Не удается создать выходную директорию {output_dir}: {e}")
    ...
    def _validate_reproducibility_config(self, config: RLConfig) -> None:
        ...
        if warnings_list:
            logger.info(f"Обнаружено {len(warnings_list)} предупреждений о воспроизводимости")
    ...
    def load_config_with_seed_validation(
        ...
        if validate_reproducibility:
            ...
            if warnings_list:
                logger.warning(f"Обнаружено {len(warnings_list)} проблем с воспроизводимостью")
                for warning_msg in warnings_list:
                    logger.warning(warning_msg)
    ...
    def validate_seed_consistency_across_configs(
        ...
        """
        logger.info(f"Проверка консистентности сидов для {len(config_paths)} конфигураций")
        ...
        for config_path in config_paths:
            try:
                ...
                seeds_info.append({
                    ...
                    "experiment_seeds": config.experiment.seeds if config.experiment else None
                })
        ...
        main_seeds = [info.get("main_seed") for info in seeds_info if "main_seed" in info]
        ...
        logger.info(f"Проверка консистентности завершена. "
...
def enforce_global_seed_consistency(
    ...
    logger.info(f"Принудительная синхронизация {len(configs)} конфигураций с сидом {master_seed}")
