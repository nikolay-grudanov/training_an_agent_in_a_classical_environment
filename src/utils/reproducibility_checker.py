...
class ReproducibilityChecker:
    ...
    def __init__(
        ...
        logger.info(
            f"Инициализирован ReproducibilityChecker (уровень: {strictness_level.value})"
        )
    ...
    def register_experiment_run(
        ...
        with open(run_file, "w", encoding="utf-8") as f:
            json.dump(
                self._run_to_dict(run), f, indent=2, ensure_ascii=False, default=str
            )
    ...
    def check_reproducibility(
        ...
        """
        logger.info(
            f"Начало проверки воспроизводимости для эксперимента: {experiment_id}"
        )
        ...
        if len(all_runs) < 2:
            raise ValueError(
                f"Недостаточно запусков для проверки воспроизводимости: {len(all_runs)}"
            )

        # Определяем эталонный запуск
        if reference_run_id:
            reference_run = next(
                (r for r in all_runs if r.run_id == reference_run_id), None
            )
        ...
        logger.info(
            f"Проверка воспроизводимости завершена. Результат: {'✓' if report.is_reproducible else '✗'}"
        )
        return report
    ...
    def validate_determinism(
        self, test_function: Callable[[], Any], seed: int, num_runs: int = 5
    ) -> Dict[str, Any]:
        ...
        if is_deterministic:
            ...
            logger.warning(
                f"✗ Функция не детерминирована ({len(unique_hashes)} уникальных результатов)"
            )

        return validation_result

    def diagnose_reproducibility_issues(
        self, experiment_id: str, deep_analysis: bool = True
    ) -> Dict[str, Any]:
        ...
        diagnosis["recommendations"] = self._generate_diagnosis_recommendations(
            diagnosis
        )

        logger.info(
            f"Диагностика завершена. Найдено проблем: {len(diagnosis['issues_found'])}"
        )
        return diagnosis
    ...
    def generate_reproducibility_guide(
        ...
        guide_sections.append(
            "# Руководство по обеспечению воспроизводимости RL экспериментов"
        )
        guide_sections.append(
        ...
        if experiment_id:
            guide_sections.extend(
                self._generate_experiment_specific_guide(experiment_id)
            )
    ...
    def _check_seed_consistency(
        ...
        report.seed_analysis = {
            ...
            "unique_seeds": len(
                set([reference_run.seed] + [r.seed for r in comparison_runs])
            ),
        ...
        for run in comparison_runs:
            if run.config_hash != reference_config_hash:
                issue = ReproducibilityIssue(
                    ...
                    details={
                        "reference_hash": reference_config_hash,
                        "run_hash": run.config_hash,
                    },
                )
                    ...
                    if not np.isclose(ref_value, run_value, rtol=rtol, atol=atol):
                        issue = ReproducibilityIssue(
                            ...
                            details={
                                ...
                                "relative_difference": abs(ref_value - run_value)
                                / max(abs(ref_value), 1e-10),
                            },
                        )
                        report.issues.append(issue)

                # Проверяем массивы
                elif isinstance(ref_value, (list, np.ndarray)) and isinstance(
                    run_value, (list, np.ndarray)
                ):
    ...
    def _check_statistical_equivalence(
        ...
        if len(comparison_runs) < min_runs - 1:
            logger.warning(
                f"Недостаточно запусков для статистического анализа: {len(comparison_runs) + 1}"
            )
            return
        ...
        if len(metric_values) >= 3:
            ...
            if len(final_values) >= 3:
                try:
                    ...
                    logger.warning(
                        f"Ошибка при выполнении статистических тестов для {metric_name}: {e}"
                    )
    ...
    def _analyze_dependencies(self, report: ReproducibilityReport) -> None:
        ...
        try:
            ...
            snapshot = self.dependency_tracker.create_dependency_snapshot(
    ...
    def _compute_final_assessment(self, report: ReproducibilityReport) -> None:
        ...
        if critical_issues > 0:
            ...
            report.confidence_score = max(
                0.0, 1.0 - (critical_issues * 0.3 + warning_issues * 0.1)
            )
        else:
            report.is_reproducible = (
                warning_issues <= 2
            )  # Допускаем небольшое количество предупреждений
    ...
    def _generate_recommendations(self, report: ReproducibilityReport) -> None:
        ...
        if ReproducibilityIssueType.SEED_MISMATCH in issue_types:
            recommendations.add(
                "Используйте одинаковые сиды для всех запусков эксперимента"
            )
            recommendations.add(
                "Настройте автоматическую синхронизацию сидов в конфигурации"
            )

        if ReproducibilityIssueType.ENVIRONMENT_DIFFERENCE in issue_types:
            recommendations.add(
                "Зафиксируйте версии всех зависимостей в requirements.txt или environment.yml"
            )
            recommendations.add(
                "Используйте контейнеризацию (Docker) для обеспечения идентичности среды"
            )

        if ReproducibilityIssueType.DEPENDENCY_CONFLICT in issue_types:
            recommendations.add(
                "Устраните конфликты зависимостей с помощью pip check или conda"
            )
        ...
        if ReproducibilityIssueType.STATISTICAL_DIFFERENCE in issue_types:
            recommendations.add(
                "Проверьте настройки детерминизма в PyTorch (deterministic=True, benchmark=False)"
            )
            recommendations.add(
                "Отключите стохастические компоненты алгоритма (например, SDE)"
            )
        ...
        if not report.is_reproducible:
            recommendations.add(
                "Используйте ReproducibilityChecker для регулярной проверки экспериментов"
            )
            recommendations.add(
                "Создайте снимок зависимостей перед началом эксперимента"
            )

        # Рекомендации по уровню строгости
        if self.strictness_level == StrictnessLevel.MINIMAL and report.issues:
            recommendations.add(
                "Рассмотрите повышение уровня строгости проверки до STANDARD"
            )
    ...
    def _save_report(self, report: ReproducibilityReport) -> None:
        ...
        report_file = (
            self.reports_dir
            / f"report_{report.experiment_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        try:
            ...
            if torch.backends.cudnn.benchmark:
                ...
                diagnosis["issues"].append(
                    "PyTorch benchmark mode включен (может нарушить детерминизм)"
                )
    ...
    def _generate_diagnosis_recommendations(
        ...
        if not torch_info.get("deterministic", True):
            recommendations.append(
                "Включите детерминистический режим PyTorch: torch.backends.cudnn.deterministic = True"
            )

        if torch_info.get("benchmark", False):
            recommendations.append(
                "Отключите benchmark режим PyTorch: torch.backends.cudnn.benchmark = False"
            )
...
def create_simple_reproducibility_test(
    ...
    def test_function(seed: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {"rewards": [total_reward], "episode_lengths": [episode_length]},
        }
...
def quick_reproducibility_check(
    experiment_id: str = "quick_test", num_runs: int = 3, seed: int = 42
) -> bool:
    ...
    try:
        ...
        logger.info(
            f"Результат проверки: {'✓' if is_reproducible else '✗'} (уверенность: {confidence:.2f})"
        )
...
def validate_experiment_reproducibility(
    config: RLConfig, num_validation_runs: int = 3
) -> bool:
    ...
    if warnings_list:
        logger.warning(
            f"Обнаружены предупреждения в конфигурации: {len(warnings_list)}"
        )
