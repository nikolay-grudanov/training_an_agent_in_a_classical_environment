...
class ExperimentLoggerAdapter(logging.LoggerAdapter):
    ...
    def log_training_step(
        ...
        **kwargs: Any,
        ...
        self.info(
            f"Training step - Episode: {episode}, Timestep: {timestep}, "
            f"Reward: {reward:.4f}",
            extra=extra,
        )
...
def setup_logging(
    ...
    if log_dir:
        ...
        # Основной файл логов
        log_filename = (
            f"training_{experiment_id}.log" if experiment_id else "training.log"
        )
        ...
        error_file = (
            log_dir / f"errors_{experiment_id}.log"
            if experiment_id
            else log_dir / "errors.log"
        )
...
def get_experiment_logger(
    experiment_id: str, base_logger: Optional[logging.Logger] = None
) -> ExperimentLoggerAdapter:
...
def configure_default_logging() -> logging.Logger:
    ...
    Returns:
        Настроенный логгер
    """
    return setup_logging(log_level=logging.INFO, console_output=True, json_format=False)
