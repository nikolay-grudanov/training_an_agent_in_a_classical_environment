...
class ExperimentManager(ABC):
    ...
    def __init__(
        ...
        for directory in [self.models_dir, self.logs_dir, self.plots_dir, 
                         self.videos_dir, self.checkpoints_dir]:
    ...
    def run(self) -> ExperimentResult:
        ...
        try:
            ...
            self.logger.warning(f"Эксперимент {self.experiment_id} прерван пользователем")
    ...
    def _handle_error(self, error: Exception) -> None:
        ...
        try:
            self.metrics_tracker.export_to_json(f"partial_metrics_{self.experiment_id}.json")
            self.logger.info("Частичные результаты сохранены")
        except Exception as save_error:
            self.logger.error(f"Не удалось сохранить частичные результаты: {save_error}")
    ...
    def _handle_cancellation(self) -> None:
        ...
        try:
            self.metrics_tracker.export_to_json(f"cancelled_metrics_{self.experiment_id}.json")
            self.logger.info("Частичные результаты отмененного эксперимента сохранены")
        except Exception as e:
            self.logger.error(f"Не удалось сохранить результаты отмененного эксперимента: {e}")
    ...
    def _create_result(self, status: str) -> ExperimentResult:
        ...
        metrics_dict = {name: summary.to_dict() for name, summary in metrics_summary.items()}
        ...
        return ExperimentResult(
            ...
            error_traceback=self.error_info.get("traceback") if self.error_info else None
        )
