...
class StatisticalTestResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'effect_size_method': self.effect_size_method.value if self.effect_size_method else None,
...
class ComparisonConfig:
    ...
    multiple_comparison_method: MultipleComparisonMethod = MultipleComparisonMethod.FDR_BH
...
class ExperimentComparator:
    ...
    def __init__(
        ...
        self.output_dir = Path(output_dir) if output_dir else Path("results/comparisons")
    ...
    def compare_experiments(
        ...
        for exp in experiments:
            try:
                ...
                self.logger.error(f"Ошибка извлечения метрик для {exp.experiment_id}: {e}")
                continue
    ...
    def statistical_significance(
        ...
        Returns:
            Результат статистического теста
        """
        if len(data1) < self.config.min_sample_size or len(data2) < self.config.min_sample_size:
            raise ValueError(f"Недостаточный размер выборки (минимум {self.config.min_sample_size})")
        ...
        if test == StatisticalTest.T_TEST:
            ...
            statistic, p_value = mannwhitneyu(data1_array, data2_array, alternative='two-sided')
        elif test == StatisticalTest.WILCOXON:
            if len(data1) != len(data2):
                raise ValueError("Для теста Уилкоксона выборки должны быть одинакового размера")
    ...
    def confidence_intervals(
        ...
        """
        if len(data) < 2:
            raise ValueError("Недостаточно данных для вычисления доверительного интервала")
            ...
            try:
                ...
                efficiency_results[exp.experiment_id] = {
                    ...
                    'sample_efficiency': (
                        ...
                        if convergence_info['convergence_timestep'] else 0
                    )
                }
            except Exception as e:
                self.logger.error(f"Ошибка анализа эффективности для {exp.experiment_id}: {e}")
                efficiency_results[exp.experiment_id] = {
    ...
    def stability_analysis(
        ...
        for exp in experiments:
            try:
                # Извлекаем данные
                metrics_history = exp.results.get('baseline', {}).get('metrics_history', [])
                values = [entry.get(metric, 0) for entry in metrics_history if metric in entry]
                ...
                self.logger.error(f"Ошибка анализа стабильности для {exp.experiment_id}: {e}")
                stability_results[exp.experiment_id] = {
                    'coefficient_of_variation': float('inf'),
                    'variance': float('inf'),
                    'stability_score': 0.0
    ...
    def final_performance(
        ...
        for exp in experiments:
            try:
                ...
                self.logger.error(f"Ошибка анализа финальной производительности для {exp.experiment_id}: {e}")
                final_performance[exp.experiment_id] = 0.0
    ...
    def peak_performance(
        ...
        for exp in experiments:
            try:
                metrics_history = exp.results.get('baseline', {}).get('metrics_history', [])
                values = [entry.get(metric, 0) for entry in metrics_history if metric in entry]
                timesteps = [entry.get('timestep', i) for i, entry in enumerate(metrics_history) if metric in entry]
                ...
                peak_timestep = timesteps[peak_idx] if peak_idx < len(timesteps) else peak_idx
                ...
                self.logger.error(f"Ошибка анализа пиковой производительности для {exp.experiment_id}: {e}")
                peak_results[exp.experiment_id] = {
                    'peak_value': 0.0,
                    'peak_timestep': 0,
                    'peak_episode': 0
        ...
        try:
            ...
            for exp in experiments:
                ...
                if len(values) >= smoothing_window:
                    ...
                    plt.plot(timesteps, smoothed, label=f"{exp.experiment_id} (smoothed)", linewidth=2)
    ...
    def generate_comparison_report(
        ...
        if output_format == 'html':
            ...
            content = json.dumps(comparison_result.to_dict(), indent=2, ensure_ascii=False)
        else:
        ...
        for format_type in formats:
            try:
                if format_type == 'csv':
                    ...
                    excel_path = self.output_dir / f"comparison_results_{timestamp}.xlsx"
    ...
    def hyperparameter_sensitivity(
        ...
        for exp in experiments:
            try:
                ...
                performance_metrics = self._extract_performance_metrics(exp, self.config)
                ...
                self.logger.error(f"Ошибка анализа чувствительности для {exp.experiment_id}: {e}")
                continue
    ...
    def environment_generalization(
        ...
        return {
            ...
            'best_environment': max(env_performance.keys(), key=lambda k: env_performance[k]['mean']) if env_performance else None,
            'worst_environment': min(env_performance.keys(), key=lambda k: env_performance[k]['mean']) if env_performance else None
        }
    ...
    def algorithm_ranking(
        ...
        if len(weights) != len(metrics):
            raise ValueError("Количество весов должно соответствовать количеству метрик")
        ...
        if algorithm_scores:
            ...
            for metric in metrics:
                ...
                if max(values) > min(values):
                    for algorithm in algorithm_scores:
                        normalized_value = (algorithm_scores[algorithm][metric] - min(values)) / (max(values) - min(values))
            ...
            # Ранжирование
            ranking = sorted(weighted_scores.keys(), key=lambda k: weighted_scores[k], reverse=True)
        else:
            weighted_scores = {}
            ranking = []
    ...
    def _extract_performance_metrics(
        ...
        if not experiment.results or 'baseline' not in experiment.results:
            raise ValueError(f"Отсутствуют результаты для эксперимента {experiment.experiment_id}")
        ...
        if episode_rewards and len(episode_rewards) >= config.convergence_window:
            ...
            moving_avg = self._calculate_moving_average(episode_rewards, config.convergence_window)
            ...
            for j in range(i + 1, len(experiment_ids)):
                ...
                try:
                    ...
                    test_result = self.statistical_significance(
                        data1.tolist(), data2.tolist(), test_type, config.significance_level
                    )
                    ...
                    self.logger.error(f"Ошибка статистического теста {exp1_id} vs {exp2_id}: {e}")
                    continue
    ...
    def _apply_multiple_comparison_correction(
        ...
        if config.multiple_comparison_method == MultipleComparisonMethod.BONFERRONI:
            ...
            for i, idx in enumerate(sorted_indices):
                corrected_p_values[idx] = all_p_values[idx] * len(all_p_values) / (i + 1)
    ...
    def _rank_experiments(
        ...
        if metrics:
            ...
            overall_ranking = sorted(
                experiment_ranks.keys(),
                key=lambda x: experiment_ranks[x]
            )
    ...
    def _extract_hyperparameter_value(
        ...
        try:
            ...
            if hyperparameter in experiment.results.get('configurations', {}).get('baseline', {}):
        ...
        for i, point1 in enumerate(points):
            ...
            for j, point2 in enumerate(points):
                if i != j:
                    ...
                    if (point2[0] >= point1[0] and point2[1] >= point1[1] and 
                        (point2[0] > point1[0] or point2[1] > point1[1])):
                        is_pareto = False
                        break
    ...
    def _create_performance_comparison_plot(
        ...
        try:
            ...
            for i, metric in enumerate(metrics):
                ...
                for exp_id in experiment_ids:
                    if exp_id in comparison_result.performance_metrics:
                        value = getattr(comparison_result.performance_metrics[exp_id], metric, 0)
        ...
        try:
            ...
            for exp_id in comparison_result.experiment_ids:
                if exp_id in comparison_result.performance_metrics:
                    ...
                    std_val = metrics.std_reward if metrics.std_reward > 0 else mean_val * 0.1
    ...
    def _create_correlation_heatmap(
        ...
        try:
            ...
            metrics_data = []
            metric_names = ['mean_reward', 'stability_score', 'sample_efficiency', 'std_reward']
    ...
    def _create_radar_chart(
        ...
        try:
            metrics = ['mean_reward', 'stability_score', 'sample_efficiency']
            experiment_ids = comparison_result.experiment_ids[:5]  # Ограничиваем количество
...
"""
        ...
        for metric, tests in comparison_result.statistical_tests.items():
            ...
            md_content += "| Comparison | Test | p-value | Significant | Effect Size |\n"
            md_content += "|------------|------|---------|-------------|-------------|\n"
        ...
        md_content += "\n## Rankings\n\n"
        for metric, ranking in comparison_result.rankings.items():
            md_content += f"**{metric.replace('_', ' ').title()}:** {' > '.join(ranking)}\n\n"
    ...
    def _format_statistical_tests_table(
        ...
        table_lines = []
        table_lines.append("Metric | Comparison | Test | Statistic | p-value | Significant | Effect Size")
        table_lines.append("-------|------------|------|-----------|---------|-------------|------------")
        
        for metric, tests in comparison_result.statistical_tests.items():
            for comparison, test_result in tests.items():
                significance = "Yes" if test_result.significant else "No"
                effect_size = f"{test_result.effect_size:.3f}" if test_result.effect_size else "N/A"
    ...
    def _export_to_csv(
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            ...
            writer.writerow([
                'experiment_id', 'mean_reward', 'std_reward', 'final_reward',
                'max_reward', 'min_reward', 'convergence_timesteps',
                'sample_efficiency', 'stability_score', 'success_rate', 'training_time'
            ])
            
            # Данные метрик производительности
            for exp_id, metrics in comparison_result.performance_metrics.items():
                writer.writerow([
                    exp_id, metrics.mean_reward, metrics.std_reward,
                    metrics.final_reward, metrics.max_reward, metrics.min_reward,
                    metrics.convergence_timesteps, metrics.sample_efficiency,
                    metrics.stability_score, metrics.success_rate, metrics.training_time
                ])
    ...
    def _export_to_excel(
        ...
        try:
            ...
            with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
                ...
                statistical_df.to_excel(writer, sheet_name='Statistical Tests', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--format", choices=['html', 'markdown', 'json'], 
                       default='html', help="Формат отчета")
