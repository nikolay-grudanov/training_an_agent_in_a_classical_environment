...
class ExperimentRunner:
        ...
        try:
            ...
            if success:
                ...
                self.experiment.stop(
                    failed=True, error_message="Ошибка выполнения конфигураций"
                )
    ...
    def handle_failure(
        ...
        if recovery_strategy == "abort":
            ...
            self.logger.warning(
                f"Неизвестная стратегия восстановления: {recovery_strategy}"
            )
            return False
    ...
    def _execute_configurations(self) -> bool:
        ...
        if self.execution_mode == ExecutionMode.VALIDATION:
            ...
            raise ValueError(
                f"Неподдерживаемый режим выполнения: {self.execution_mode}"
            )
    ...
    def _execute_parallel(self) -> bool:
        ...
        success = (
            ...
            and self.baseline_result.success
            and self.variant_result
            and self.variant_result.success
        )
    ...
    def _compare_results(self) -> None:
        ...
        self.logger.info(
            ...
            extra={
                ...
                "improvement": comparison.get("performance_metrics", {})
                .get("mean_reward", {})
                .get("improvement", 0),
            },
        )
    ...
    def _check_resource_limits(self) -> None:
        ...
        if self.resource_usage.memory_mb > self.resource_limits.get(
            "memory_mb", float("inf")
        ):
            self.logger.warning(
        ...
        if self.resource_usage.cpu_percent > self.resource_limits.get(
            "cpu_percent", 100
        ):
            self.logger.warning(
    ...
    def _save_final_state(self) -> None:
        ...
        final_state = {
            ...
            "results": {
                ...
                "variant": self.variant_result.to_dict()
                if self.variant_result
                else None,
            },
...
def run_experiment_cli(
    ...
    try:
        ...
        if config:
            ...
            experiment_path = (
                Path(output_dir) / experiment_id / f"experiment_{experiment_id}.json"
            )
            if not experiment_path.exists():
                click.echo(
                    f"❌ Эксперимент {experiment_id} не найден в {experiment_path}"
                )
                sys.exit(1)
        ...
        if success:
            ...
            if runner.baseline_result and runner.variant_result:
                click.echo(
                    f"📈 Baseline награда: {runner.baseline_result.final_mean_reward:.2f}"
                )
                click.echo(
                    f"📈 Variant награда: {runner.variant_result.final_mean_reward:.2f}"
                )

                improvement = (
                    runner.variant_result.final_mean_reward
                    - runner.baseline_result.final_mean_reward
                )
